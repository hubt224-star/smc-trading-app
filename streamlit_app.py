...
st.set_page_config(page_title="Pro SMC Trading Terminal", layout="wide")

# Har 5 second me auto-refresh
count = st_autorefresh(interval=5000, limit=1000, key="fno_auto_refresh")

st.title("Pro SMC Trading Terminal (NSE, BSE, F&O, Commodity)")
st.caption(f"Live Auto-Refresh (5s) | Cycles: {count}")
...
if not data.empty:
    ...
    # SMC Trend & Signal Logic
    data['Signal'] = "NO TRADE ZONE"
    
    # Buy only above 200 EMA (Bullish Trend), Sell only below 200 EMA (Bearish Trend)
    buy_cond = (data['Close'] > data['Pivot_High'].shift(1)) & (data['Close'] > data['EMA200'])
    sell_cond = (data['Close'] < data['Pivot_Low'].shift(1)) & (data['Close'] < data['EMA200'])
    ...
    sl = latest_price - (1.5 * latest_atr) if latest_signal == "BUY (CE)" else latest_price + (1.5 * latest_atr)
    target = latest_price + (3.0 * latest_atr) if latest_signal == "BUY (CE)" else latest_price - (3.0 * latest_atr)
    ...
    if latest_signal == "BUY (CE)":
        col2.success(f"**SIGNAL: {latest_signal}**")
        ...
        col2.error(f"**SIGNAL: {latest_signal}**")
        ...
        col2.warning(f"**SIGNAL: {latest_signal}**")
        
    col3.metric("Est. Stop-Loss", f"{sl:.2f}")
    col4.metric("Est. Target (1:2)", f"{target:.2f}")
    ...
    st.dataframe(data[['Open', 'High', 'Low', 'Close', 'ATR', 'Signal']].tail(10).style.map(color_signals, subset=['Signal']))
else:
